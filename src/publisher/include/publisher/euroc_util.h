...
public:
    struct frame {
        frame(const std::string& left_img_path, const std::string& right_img_path, const double timestamp,int sec,unsigned long nsec)
            : left_img_path_(left_img_path), right_img_path_(right_img_path), timestamp_(timestamp), sec_(sec), nsec_(nsec){};

        const std::string left_img_path_;
        const std::string right_img_path_;
        const double timestamp_;
        const int sec_;
        const unsigned long nsec_;
    };
...
private:
    ...
    std::vector<int> secs_;
    std::vector<unsigned long> nsecs_;
};
